...
CREATE TABLE IF NOT EXISTS `senicare`.`nurses` (
  ...
  `sns_id` VARCHAR(255) NULL COMMENT 'OAuth2 id',
  PRIMARY KEY (`user_id`),
  UNIQUE INDEX `user_id_UNIQUE` (`user_id` ASC) VISIBLE,
  UNIQUE INDEX `tel_number_UNIQUE` (`tel_number` ASC) VISIBLE,
  CONSTRAINT ` tel_auth_number`
...
CREATE TABLE IF NOT EXISTS `senicare`.`customers` (
  `custom_number` INT NOT NULL AUTO_INCREMENT COMMENT '고객번호',
  ...
  `charger` VARCHAR(20) NOT NULL COMMENT '담당요양사',
...
CREATE TABLE IF NOT EXISTS `senicare`.`care_records` (
  ...
  INDEX `record_chager_idx` (`charger` ASC) VISIBLE,
